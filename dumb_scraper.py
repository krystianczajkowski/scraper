...
def validate_input():
    ...
    board_list = ('a', 'c', 'w', 'm', 'cgl', 'cm', 'n', 'jp', 'vp', 'v', 'vg', 'vr', 'co', 'g', 'tv', 'k', 'o', 'an', 'tg', 'sp', 'asp', 'sci', 'int', 'out', 'toy', 'biz', 'i', 'po', 'p', 'ck', 'ic',
...
def get_thread_no(filename):
    ...
    with open(filename) as f:
        data = json.load(f)
        to_save = [data[i]['threads'][j]['no'] for i, _ in enumerate(
            data) for j, e in enumerate(data[i]['threads']) if data[i]['threads'][j]['replies'] > 1]
...
def get_thread_contents(path):
    ...
    with open(path) as f:
        for line in f.readlines():
            r = requests.get(
                f'https://a.4cdn.org/{sys.argv[1]}/thread/{line.strip()}.json')
            data = r.json()
